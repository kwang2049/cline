...
import { configureApiFromEnvironment } from "./configure-api"

async function main() {
	log("Starting simplified standalone service...")

	// Initialize without host providers to avoid host bridge dependency
	activate(extensionContext)

	// Configure API from environment variables
	await configureApiFromEnvironment()
...
function startProtobusService(controller: Controller) {
	...
	server.bindAsync(host, grpc.ServerCredentials.createInsecure(), (err) => {
		...
		log("Simplified standalone server ready for CLI connections")
	})
}
